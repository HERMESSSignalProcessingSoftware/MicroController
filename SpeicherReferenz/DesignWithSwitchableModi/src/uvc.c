...
#include "../inc/facetAndVisionConfiguration.h"
...
#include "../inc/camera_ptzcontrol.h"
#include "../inc/cyfxgpif2config.h"
...
#define  USB3_UVC_FPS                   15
#define  USB3_UVC_FPS_REG               (10^7/USB3_UVC_FPS)
#define  USB3_UVC_MAX_VIDEO_FRAME_SIZE  FACETANDVISION_SONY_X_RESOLUTION*FACETANDVISION_SONY_Y_RESOLUTION
...
uint8_t glProbeCtrl[CY_FX_UVC_MAX_PROBE_SETTING] = {
    ...
    CUT0B(USB3_UVC_FPS_REG),    // Desired frame interval in the unit of 100ns:
    CUT1B(USB3_UVC_FPS_REG),
    CUT2B(USB3_UVC_FPS_REG),
    CUT3B(USB3_UVC_FPS_REG),
    ...
    CUT0B(USB3_UVC_MAX_VIDEO_FRAME_SIZE),    //  Max video frame size in bytes
    CUT1B(USB3_UVC_MAX_VIDEO_FRAME_SIZE),
    CUT2B(USB3_UVC_MAX_VIDEO_FRAME_SIZE),
    CUT3B(USB3_UVC_MAX_VIDEO_FRAME_SIZE),
    0x00, 0x40, 0x00, 0x00      /* No. of bytes device can rx in single payload = 16 KB */
};

#define USB2_UVC_FPS                  15
#define USB2_UVC_FPS_REG              (10^7/USB2_UVC_FPS)
//#define USB2_UVC_MAX_VIDEO_FRAME_SIZE 614400
#define USB2_UVC_MAX_VIDEO_FRAME_SIZE 2*1920*1440
...
uint8_t glProbeCtrl20[CY_FX_UVC_MAX_PROBE_SETTING] = {
    ...
    0x01,                       /* Use 1st Video format index */
    0x01,                       /* Use 1st Video frame index */
    CUT0B(USB2_UVC_FPS_REG),    // Desired frame interval in the unit of 100ns:
    CUT1B(USB2_UVC_FPS_REG),
    CUT2B(USB2_UVC_FPS_REG),
    CUT3B(USB2_UVC_FPS_REG),
    0x00, 0x00,                 /* Key frame rate in key frame/video frame units: only applicable
                                   to video streaming with adjustable compression parameters */
    0x00, 0x00,                 /* PFrame rate in PFrame / key frame units: only applicable to
                                   video streaming with adjustable compression parameters */
    ...
    0x00, 0x00,                 /* Window size for average bit rate: only applicable to video
                                   streaming with adjustable compression parameters */
    0x00, 0x00,                 /* Internal video streaming i/f latency in ms */
    CUT0B(USB2_UVC_MAX_VIDEO_FRAME_SIZE),    //  Max video frame size in bytes
    CUT1B(USB2_UVC_MAX_VIDEO_FRAME_SIZE),
    CUT2B(USB2_UVC_MAX_VIDEO_FRAME_SIZE),
    CUT3B(USB2_UVC_MAX_VIDEO_FRAME_SIZE),
    0x00, 0x40, 0x00, 0x00      /* No. of bytes device can rx in single payload = 16 KB */
};
...
volatile static uint32_t FrameSize = 0;
#endif
...
{
	functionInformationPrint();
    switch (evtype)
    {
        case CY_U3P_USB_EVENT_RESET:
        	informationPrint("RESET encountered...");
        ...
        case CY_U3P_USB_EVENT_SUSPEND:
        	informationPrint("SUSPEND encountered...");
            CyU3PGpifDisable (CyFalse);
            streamingStarted = CyFalse;
            CyFxUVCApplnAbortHandler ();
            break;
        ...
        case CY_U3P_USB_EVENT_DISCONNECT:
        	informationPrint("USB disconnected...");
            CyU3PGpifDisable (CyFalse);
            streamingStarted = CyFalse;
            CyFxUVCApplnAbortHandler ();
            break;
        ...
        case CY_U3P_USB_EVENT_EP_UNDERRUN:
        	informationPrint("CY_U3P_USB_EVENT_EP_UNDERRUN encountered...");
            break;
                ...
                case CY_FX_UVC_CONTROL_INTERFACE:
                    {
                        ...
                        {
                        	errorPrint("Set CY_FX_UVC_VIDEO_CONTROL_REQUEST_EVENT Failed %x", status);
                            CyU3PUsbStall (0, CyTrue, CyFalse);
                        }
                        else
                        	informationPrint("Set CY_FX_UVC_VIDEO_CONTROL_REQUEST_EVENT successful");
                    }
                    break;
                ...
                case CY_FX_UVC_STREAM_INTERFACE:
                    {
                        ...
                        {
                            /* Error handling */
                        	errorPrint("Set CY_FX_UVC_VIDEO_STREAM_REQUEST_EVENT Failed %x\n", status);
                            CyU3PUsbStall (0, CyTrue, CyFalse);
                        }
                        else
                        	informationPrint("Set CY_FX_UVC_VIDEO_STREAM_REQUEST_EVENT successful");
                    }
                    break;
        ...
        case CY_FX_USB_SET_INTF_REQ_TYPE:
            if (bRequest == CY_FX_USB_SET_INTERFACE_REQ)
            {
                ...
                if ((wIndex == CY_FX_UVC_STREAM_INTERFACE) && (wValue == 0))
                {
                	...
                	informationPrint("Alternate setting 0..");
        ...
        case CY_U3P_USB_TARGET_ENDPT:
            if (bRequest == CY_U3P_USB_SC_CLEAR_FEATURE)
            {
                if (wIndex == CY_FX_EP_BULK_VIDEO)
                {
                    ...
                    {
                    	informationPrint("Clear feature request detected..");
        ...
        )
{
    ...
    {
        errorPrint("DMA Hardware has detected an Error");
    }
    ...
    if (type == CY_U3P_DMA_CB_PROD_EVENT)
    {
        ...
        while (status == CY_U3P_SUCCESS)
        {
...
#ifdef DEBUG_PRINT_FRAME_COUNT
                glFrameCount++;
                FrameSize = (glDmaDone - 1) * CY_FX_UVC_BUF_FULL_SIZE + dmaBuffer.count;
                glDmaDone = 0;
#endif
            }
            ...
            {
            	errorPrint("CyU3PDmaMultiChannelCommitBuffer code %d", status);
                break;
            }
        ...
        void)
{
    ...
    {
      errorPrint("UART initialization failed!");
      CyFxAppErrorHandler(apiRetStatus);
    }
    else
    	informationPrint("Debug initialisation was successful!");
...
CyFxUVCApplnI2CInit (void)
{
    ...
    if (status != CY_U3P_SUCCESS)
    {
    	errorPrint("I2C initialization failed!");
        CyFxAppErrorHandler (status);
    }
    else
    	informationPrint("I2C initialization successful!");
    	//CyU3PDebugPrint (4, "%s:I2C initialization successful!\r\n",__func__);
    ...
    {
    	errorPrint("I2C configuration failed!");
        CyFxAppErrorHandler (status);
    }
    else
    	informationPrint("I2C configuration successful!");
}
        ...
        uint16_t cbArg)
{
    if ((cbType == CYU3P_PIB_INTR_ERROR) && ((cbArg == 0x1005) || (cbArg == 0x1006)))
    {
        ...
        {
            errorPrint("Backflow detected...\n\r\t" \
                "that means the DMA buffer are getting overflowed.\n\r\t" \
                "This happens if the host is not requesting data as quickly as the buffers are getting filled");
        ...
        void)
{
    ...
    {
        /* Error Handling */
    	errorPrint("Loading GPIF Configuration failed, Error Code = %d", apiRetStatus);

    	if(apiRetStatus == CY_U3P_ERROR_NOT_SUPPORTED)
    	  errorPrint("Loading GPIF Configuration failed CY_U3P_ERROR_NOT_SUPPORTED");
    	else if(apiRetStatus == CY_U3P_ERROR_ALREADY_STARTED)
        errorPrint("Loading GPIF Configuration failed CY_U3P_ERROR_ALREADY_STARTED");
    	else if(apiRetStatus == CY_U3P_ERROR_BAD_ARGUMENT)
        errorPrint("Loading GPIF Configuration failed CY_U3P_ERROR_BAD_ARGUMENT");

        CyFxAppErrorHandler (apiRetStatus);
    }
    else
    	informationPrint("Loading GPIF Configuration successful");
}
...
CyFxUVCApplnInit (void)
{
    ...
    if (apiRetStatus != 0)
    {
    	errorPrint("UVC Create Event failed, Error Code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    else
    	informationPrint("UVC Create Event was successful");
    ...
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    	errorPrint("PIB Function Failed to Start, Error Code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    else
    	informationPrint("PIB Function is started successful");
    ...
    informationPrint("waiting until FPGA is ready");

    uint8_t uint8_tmp=0;
    apiRetStatus =  CY_U3P_ERROR_DELETED;
    while(uint8_tmp != 1 || apiRetStatus !=  CY_U3P_SUCCESS)
      apiRetStatus = fpgaReadI2CRegister(128,&uint8_tmp);

    fpgaWriteI2CRegister(0,CUT1B(FACETANDVISION_SONY_X_RESOLUTION));
    fpgaWriteI2CRegister(1,CUT0B(FACETANDVISION_SONY_X_RESOLUTION));
    fpgaWriteI2CRegister(2,CUT1B(FACETANDVISION_SONY_HORIZONTAL_CONTAINER_SIZE));
    fpgaWriteI2CRegister(3,CUT0B(FACETANDVISION_SONY_HORIZONTAL_CONTAINER_SIZE));
    fpgaWriteI2CRegister(4,CUT1B(FACETANDVISION_SONY_Y_RESOLUTION));
    fpgaWriteI2CRegister(5,CUT0B(FACETANDVISION_SONY_Y_RESOLUTION));
    fpgaWriteI2CRegister(6,CUT1B(FACETANDVISION_SONY_Y_RESOLUTION + 3));
    fpgaWriteI2CRegister(7,CUT0B(FACETANDVISION_SONY_Y_RESOLUTION + 3));
    fpgaWriteI2CRegister(8,FACETANDVISION_CONFIGURATION_MOD);
    informationPrint("start to configurate the sensor");

    uint8_t channels=0;
    for(int i=FACETANDVISION_START_NUMBER_OF_SENSORS; i<FACETANDVISION_MAX_NUMBER_OF_SENSORS;i++)
    {
      uint8_t shiftvalue = 1<<i;
      apiRetStatus = TCA9546A_enableChannels(&shiftvalue);
      channels = channels + shiftvalue;
      apiRetStatus = SensorSonyIMX214SoftwareReset();
      CyU3PThreadSleep(100); //wait until the sensor reset is done
      SensorSonyIMX214InitParam SensorSonyIMX214InitParameter;
      SensorSonyIMX214InitParameter.resolutionHorizontalContainerSize = FACETANDVISION_SONY_HORIZONTAL_CONTAINER_SIZE;
      SensorSonyIMX214InitParameter.resolutionVerticalContainerSize = FACETANDVISION_SONY_VERTICAL_CONTAINER_SIZE;
      SensorSonyIMX214InitParameter.uint16_xleftCorner = 0;
      SensorSonyIMX214InitParameter.uint16_yleftCorner = 0;
      SensorSonyIMX214InitParameter.uint16_xrightCorner = FACETANDVISION_SONY_X_RIGHT_CORNER_PIXEL;
      SensorSonyIMX214InitParameter.uint16_yrightCorner = FACETANDVISION_SONY_Y_RIGHT_CORNER_PIXEL;
      SensorSonyIMX214InitParameter.binningMode = FACETANDVISION_SONY_BINNING;
      SensorSonyIMX214InitParameter.binningWeighting = FACETANDVISION_SONY_WEIGHTING;
      SensorSonyIMX214InitParameter.bitDepth = SENSOR_SONY_IMX214_CSI_DT_FMT_L;
      SensorSonyIMX214InitParameter.dataRate = SENSOR_SONY_IMX214_REQ_LINK_RATE_MBPS_U16_U16;
      SensorSonyIMX214InitParameter.horizontalSize = FACETANDVISION_SONY_X_RESOLUTION;
      SensorSonyIMX214InitParameter.verticalSize = FACETANDVISION_SONY_Y_RESOLUTION;
      SensorSonyIMX214InitParameter.coarseIntegTime = 1000;
      SensorSonyIMX214InitParameter.gain.analogGainGlobal = 0;
      SensorSonyIMX214InitParameter.gain.digitalGainGreen = 256;
      SensorSonyIMX214InitParameter.gain.digitalGainRed = 256;
      SensorSonyIMX214InitParameter.gain.digitalGainBlue = 256;
      SensorSonyIMX214InitParameter.gain.digitalGainGb = 256;
      SensorSonyIMX214InitParameter.gain.analogueGainGlobal = 0;
      SensorSonyIMX214Init(&SensorSonyIMX214InitParameter);
    }
    CyU3PBusyWait(4000); //wait until the sensor reset is done
    ...
    TCA9546A_enableChannels(&channels);
    ...
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    	errorPrint("USB Function Failed to Start, Error Code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    else
    	informationPrint("USB Function is started successful");
    ...
    {
        /* Error Handling */
    	errorPrint("USB Set Endpoint config failed, Error Code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    else
    	informationPrint("USB Set Endpoint config successful");
    ...
    {
        /* Error handling */
    	errorPrint("DMA Channel Creation Failed, Error Code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    else
    	informationPrint("DMA Channel Creation successful");
    ...
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    	errorPrint("Debug Command endpoint config failed, Error code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    ...
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    	errorPrint("Debug Response endpoint config failed, Error code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    ...
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    	errorPrint("Debug Command channel create failed, Error code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    ...
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    	errorPrint("Debug channel SetXfer failed, Error code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    ...
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    	errorPrint("Debug Response channel create failed, Error code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    ...
    {
    	errorPrint("Failed to allocate memory for debug buffer");
    ...
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    	errorPrint("USB Connect failed, Error Code = %d", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    }
    else
    	informationPrint("USB Connect successful");

    usbSpeed = CyU3PUsbGetSpeed ();
    if(usbSpeed == CY_U3P_SUPER_SPEED)
        informationPrint("USB3 connection was detected");
    else
        informationPrint("USB2 connection was detected");

    informationPrint("Function CyFxUVCApplnInit done");
}
...
(void)
{
  ...
  if (apiRetStatus != CY_U3P_SUCCESS)
  {
    errorPrint("CyU3PDmaMultiChannelSetXfer  = %d\n", apiRetStatus);
  }
  else
    informationPrint("DMA Channel Set Transfer successful");
  ...
  if (apiRetStatus != CY_U3P_SUCCESS)
  {
    errorPrint("CyU3PGpifSMStart  = %d\n", apiRetStatus);
  }
  else
    informationPrint("Starting GPIF state machine successful");

  apiRetStatus = CyU3PGpifSMSwitch (257, 0, 257, 0, 2);

  if (apiRetStatus != CY_U3P_SUCCESS)
  {
    errorPrint("CyU3PGpifSMSwitch  = %d\n", apiRetStatus);
  }
    ...
    for (;;)
    {
        ...
        if (apiRetStatus == CY_U3P_SUCCESS)
        {
            ...
            if ((flag & CY_FX_UVC_STREAM_EVENT) != 0)
            {
                ...
                {
                    /* Error handling */
                	errorPrint("DMA Channel Set Transfer Failed, Error Code = %d", apiRetStatus);
                    CyFxAppErrorHandler (apiRetStatus);
                }
                else
                	informationPrint("DMA Channel Set Transfer successful");
                ...
                {
                    /* Error Handling */
                	errorPrint("Starting GPIF state machine failed, Error Code = %d", apiRetStatus);
                    CyFxAppErrorHandler (apiRetStatus);
                }
                else
                	informationPrint("Starting GPIF state machine successful");
...
#ifdef DEBUG_PRINT_FRAME_COUNT
        informationPrint("UVC: Completed %d frames and %d buffers", glFrameCount,
                (glDmaDone != 0) ? (glDmaDone - 1) : 0);
        informationPrint("The Image of PFGA is %d bytes large", FrameSize);
        informationPrint("Number of Lines in Image of PFGA is %d", FrameSize/(FACETANDVISION_USB_OUTPUT_WIDTH*2));
#endif

#ifdef DEBUG_PRINT_FPGA_RESOLUTION
       uint8_t uint8_tmp;
        for(int i=4; i < 9; i++)
        {
          fpgaReadI2CRegister(i,&uint8_tmp);

          if(i == 4)
          {
            informationPrint("");
            informationPrint("Horizontal Resolution");
          }
          else if (i == 6)
            informationPrint("Vertical Resolution");
          else if (i == 8)
            informationPrint("Mipi Error Code");

          informationPrint("read from fpga address %d the value %x", i, uint8_tmp);
        }
#endif
    }
}
            ...
            switch (bRequest)
            {
                ...
                case CY_FX_USB_UVC_SET_CUR_REQ:
                    ...
                    if (apiRetStatus == CY_U3P_SUCCESS)
                    {
                        ...
                        if (apiRetStatus != CY_U3P_SUCCESS)
                        {
                        	errorPrint("UVCHandleVideoStreamingRqts:Set CY_FX_UVC_STREAM_EVENT failed %x", apiRetStatus);
                        }
                        else
                        	informationPrint("UVCHandleVideoStreamingRqts:Set CY_FX_UVC_STREAM_EVENT successful");
                    }
                    break;
    ...
    for (;;)
    {
                    ...
                    CYU3P_WAIT_FOREVER) == CY_U3P_SUCCESS)
        {
            ...
            if (eventFlag & CY_FX_USB_DEBUG_CMD_EVENT)
            {
                ...
                if (apiRetStatus != CY_U3P_SUCCESS)
                {
                	errorPrint("Failed to receive debug command, Error code = %d", apiRetStatus);
                    CyFxAppErrorHandler (apiRetStatus);
                }
                ...
                if (dmaInfo.buffer[0] == 0)
                {
                    if (dmaInfo.count == 3)
                    {
                        glDebugRspBuffer[0] = SensorRead2B (I2C_SONY_SENSOR_ADDRESS_READ, dmaInfo.buffer[1], dmaInfo.buffer[2],
                        		(glDebugRspBuffer+1));
                        dmaInfo.count = 3;
                    }
                    else if (dmaInfo.count == 4)
                    {
                        ...
                        {
                                glDebugRspBuffer[0] = SensorRead (I2C_SONY_SENSOR_ADDRESS_READ, dmaInfo.buffer[1], dmaInfo.buffer[2],
                ...
                {
                        glDebugRspBuffer[0] = SensorWrite (I2C_SONY_SENSOR_ADDRESS_WRITE, dmaInfo.buffer[1], dmaInfo.buffer[2],
                ...
                if (apiRetStatus != CY_U3P_SUCCESS)
                {
                	errorPrint("Failed to free up command OUT EP buffer, Error code = %d", apiRetStatus);
                    CyFxAppErrorHandler (apiRetStatus);
                }
                ...
                if (apiRetStatus != CY_U3P_SUCCESS)
                {
                	errorPrint("Failed to send debug response, Error code = %d", apiRetStatus);
                    CyFxAppErrorHandler (apiRetStatus);
                }
            }
#endif
...
fatalErrorHandler:
    ...
    /* Loop indefinitely */
	failurePrint("fatalErrorHandler ...");
    while (1);
}
...
#if FACETANDVISION_GPIF2_INTERFACE_WIDTH == 8
    io_cfg.isDQ32Bit        = CyFalse;
#elif FACETANDVISION_GPIF2_INTERFACE_WIDTH == 16
    io_cfg.isDQ32Bit        = CyFalse;
#else
